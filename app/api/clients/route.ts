...
import {
  ...
  clientInputSchema,
  clientSchema,
} from "@/src/schemas/clients/clientSchema";
import { mapSupabaseError } from "@/src/helpers/getErrorMessage";

const createSchema = clientInputSchema;

const updateSchema = z
  .object({
    name: createSchema.shape.name.optional(),
    phone: createSchema.shape.phone.optional(),
  })
...
export async function GET() {
  ...
  const parsed = ClientArraySchema.safeParse(data ?? []);
  if (!parsed.success) {
