...
async function nestAuthedFetch(path: string): Promise<Response | null> {
...
export async function getClientPortalContextFromSession(): Promise<ClientPortalContext | null> {
  const authRes = await nestAuthedFetch("auth/me");
  if (!authRes?.ok) {
    return null;
  }
  ...
  const meRes = await nestAuthedFetch("client/me");
  if (!meRes?.ok) {
    return null;
  }
...
export async function getClientPortalAppointments(context: ClientPortalContext) {
  const res = await nestAuthedFetch("client/appointments");
  if (!res?.ok) {
...
export async function getClientPortalDiscounts(context: ClientPortalContext) {
  const res = await nestAuthedFetch("client/discounts");
  if (!res?.ok) {
...
export async function touchClientPortalSession(_context: ClientPortalContext) {
  await nestAuthedFetch("client/me");
}
